...
export const SettingsPage: React.FC = () => {
  ...
  const handleSaveKey = async () => {
    try {
      ...
      alert('Failed to save API key. Make sure you are logged in as admin.');
    }
  };

  const clearAllData = () => {
    if (confirm('WARNING: This will delete ALL your local data. This cannot be undone. Are you sure?')) {
      localStorage.clear();
    ...
    <div className="pb-24 pt-4 px-4 max-w-2xl mx-auto space-y-8">
      ...
      {isAdmin ? (
        <section className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 space-y-6">
          <div className="flex items-center gap-3 mb-2">
             ...
             <h2 className="text-xl font-bold text-white">System Settings (Admin)</h2>
          </div>
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 space-y-6">
          ...
          <div className="p-4 bg-zinc-950 rounded-2xl border border-zinc-800">
            <p className="text-sm text-zinc-400">
              The DeepSeek API key is managed by your administrator. Please contact them if you are experiencing issues with AI responses.
            </p>
          </div>
          <a 
            href="https://platform.deepseek.com/" 
          ...
          >
            Learn about DeepSeek <ExternalLink size={12} />
          </a>
        </section>
      )}
      ...
      <section className="bg-zinc-900 border border-red-900/30 rounded-3xl p-6 space-y-6">
        ...
        <div>
          ...
          >
            <Trash2 size={20} />
            Reset All Local Data & Clear Cache
          </button>
          <p className="mt-4 text-xs text-center text-zinc-600">
            This clears your local session. Server data (scenarios, chats) is unaffected.
          </p>
        </div>
      </section>

      <div className="text-center pt-8">
         <p className="text-xs text-zinc-700 font-bold uppercase tracking-[0.2em]">FictionLab v1.0.0</p>
      </div>
    </div>
  );
};
